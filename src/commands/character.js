    ...
    try {
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: '🗣️ Voice', value: characterConfig.voice_config?.voice_id || 'Default', inline: true },
        ...
        await interaction.reply({
            content: `❌ Failed to set character "${characterName}": ${error.message}`,
            ephemeral: true,
        });
    }
}
...
async function handleCurrent(interaction) {
    ...
    if (!characterConfig) {
        return await interaction.reply({
            content: `❌ Current character "${currentCharacter}" configuration not found.`,
            ephemeral: true,
        });
    }
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields(
            { name: '🗣️ Voice ID', value: characterConfig.voice_config?.voice_id || 'Default', inline: true },
...
async function handleReset(interaction) {
    ...
    try {
        ...
        if (wasCleared) {
            ...
            embed
                .setTitle('ℹ️ No Context to Reset')
        ...
        await interaction.reply({
            content: '❌ Failed to reset character context. Please try again.',
            ephemeral: true,
        });
    }
}